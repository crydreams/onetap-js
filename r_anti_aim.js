...
var path = ["Rage", "Anti aim", "Anti aim"];
...
UI.AddHotkey(kpath, "low delta", "low delta");
UI.AddCheckbox(path, " jitter when running");
UI.AddCheckbox(path, " use inverter");
UI.AddCheckbox(path, "Indicate cheat ");
...
var yawbasecache = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw base"]);
function onCreateMove() {
        if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Enabled"])) {
            ...
            //Yaw base
            switch (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw base"])) {
                case 0:
            ...
            //Yaw
            if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter"]) == 0) {
                SetRealYaw = UI.GetValue(["Rage", "Anti aim", "CAnti aim", "Yaw"]);
            }

            //Yaw jitter
            else {
                if (Globals.ChokedCommands() == 0) {
                    if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter"]) == 1) {
                        if (RealSwitch) {
                            SetRealYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw"]);
                        } else {
                            SetRealYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw"]) + UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter value"]);
                        }
                        RealSwitch = !RealSwitch;
                    } else if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter"]) == 2) {
                        if (RealSwitch) {
                            SetRealYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw"]) - UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter value"]) / 2;
                        } else {
                            SetRealYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw"]) + UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter value"]) / 2;
                        }
                        RealSwitch = !RealSwitch;
                    } else if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter"]) == 3) {
                        SetRealYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw"]) + RandomInt(UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter value"]) / -2, UI.GetValue(["Rage", "Anti aim", "Anti aim",  "Yaw jitter value"]) / 2);
                    }
                }
            }
            ...
            //Body Yaw
            if (Globals.ChokedCommands() == 0) {
            if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Enable jitter when running"]) && getVelocity() > 110) {
              if (FakeSwitch) {
  							SetFakeYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Body yaw value"]);
  						} else {
  							SetFakeYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Body yaw value"]) + 60;
  						}
  						FakeSwitch = !FakeSwitch;
            }
            else {

                  if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Body yaw"]) == 0) {
  					SetFakeYaw = 120;
                  } else if (UI.GetValue(["Rage", "Anti aim", "Anti aim",  "Body yaw"]) == 1) {
                      SetFakeYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Body yaw value"]);
                  } else if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Body yaw"]) == 2) {
  					if (!UI.GetValue(["Config", "Scripts", "Keys", "JS Keybinds", "low delta"])) {
  						if (FakeSwitch) {
  							SetFakeYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Body yaw value"]);
  						} else {
  							SetFakeYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Body yaw value"]) + 60;
            ...
            //LBY
            //Freestanding
            if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Freestanding"]) && UI.GetValue(["Config", "Scripts", "Keys", "JS Keybinds", "Freestanding hotkey"])) {
                UI.SetValue(["Rage", "Anti Aim", "Directions", "Auto direction"], 1);
            } else {
                UI.SetValue(["Rage", "Anti Aim", "Directions", "Auto direction"], 0);
            }
            if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Freestand body yaw"]) != 0 && UI.GetValue(["Config", "Scripts", "Keys", "JS Keybinds", "Freestanding body yaw hotkey"])) {
                ...
                if (left_distance < 600 && right_distance < 600) {
                    if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Freestand body yaw"]) == 2) {
            ...
            if(isLeftActive && leftWasPressed == false) {
                ...
                drawLeft = 1;
                drawRight = 0;
                UI.SetValue( ["Rage", "Anti aim", "Yaw"], -90 );
                UI.SetValue( ["Rage", "Anti aim", "Yaw base"], 0 );
            ...
            if (isHideRealActive) {
                ...
                drawLeft = 0;
                drawRight = 0;
                UI.SetValue( ["Rage", "Anti aim", "Yaw base"], 1 );
                UI.SetValue( ["Rage", "Anti aim", "Yaw"], 0 );
            }
            
            //Inverter
            if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Allow to use inverter"])) {
            ...
            //Main part
            SetFakeYaw = clamp(SetFakeYaw, UI.GetValue(["Rage", "Anti aim", "Anti aim", "Fake yaw limit"]) * -1, UI.GetValue(["Rage", "Anti aim", "Anti aim", "Fake yaw limit"]));
			if (UI.GetValue(["Config", "Scripts", "Keys", "JS Keybinds", "low delta"])) {
                if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Lower body yaw target"]) == 1) {
					SetFakeYaw = (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Expose fake when exploiting"]) && UI.GetValue(["Rage", "Exploits", "Keys", "Key assignment", "Double tap"])) ? -42 : 42;
            }
            if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Lower body yaw target"]) == 2) {
					SetFakeYaw = (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Expose fake when exploiting"]) && UI.GetValue(["Rage", "Exploits", "Keys", "Key assignment", "Double tap"])) ? 30 : 30;
            }
        }

			if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Lower body yaw target"]) == 0) {
                if (Math.floor(Globals.Curtime()) % 5 > 2) {
					SetFakeYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Fake yaw limit"]);
					SetLBYYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Fake yaw limit"]) * 2;
				}
				else {
					SetFakeYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Fake yaw limit"]) * -1;
					SetLBYYaw = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Fake yaw limit"]) * -2;
				}
            } else if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Lower body yaw target"]) == 1) {
                SetLBYYaw = SetFakeYaw * 2;
            } else if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Lower body yaw target"]) == 2) {
                SetLBYYaw = SetFakeYaw;
            }
            //SetLBYYaw = clamp(SetLBYYaw, UI.GetValue(["Rage", "Anti aim", "Anti aim", "Fake yaw limit") * -2, UI.GetValue(["Rage", "Anti aim", "Anti aim", "Fake yaw limit") * 2);
...
function onDraw() {
        ...
        var offset = 0;
        var flip = false;
        if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Anti-aim display mode"]) == 1) {
            if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Freestand body yaw"]) != 0 && UI.GetValue(["Config", "Scripts", "Keys", "JS Keybinds", "Freestanding body yaw hotkey"])) {
                if (left_distance < 600 && right_distance < 600) {
                    if (fontalpha < 1)
            ...
            if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Freestand body yaw"]) == 0) {
                    if (UI.GetValue(["Rage", "Anti Aim", "General", "Key assignment", "AA Direction inverter"]))
                        flip = true;
                    else
                        flip = false;
                ...
                if (UI.GetValue(["Config", "Scripts", "Keys", "JS Keybinds", "Freestanding body yaw hotkey"])) {
                    if (left_distance < right_distance && UI.GetValue(["Rage", "Anti aim", "Anti aim", "Freestand body yaw"]) != 0)
                        flip = false;
                    else
                        flip = true;
                }
        ...
        if (UI.GetValue(["Rage", "Exploits", "Keys", "Key assignment", "Double tap"]) && UI.GetValue(["Rage", "Anti aim", "Anti aim", "Expose fake when exploiting"])) {
            if (!dtactive) {
                dtactive = true;
                cachey = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw"]);
                cacheyj = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter"]);
                cacheyjv = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter value"]);
                cacheby = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Body yaw"]);
                cachebyv = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Body yaw value"]);
                cachelby = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Lower body yaw target"]);
                cachefyl = UI.GetValue(["Rage", "Anti aim", "Anti aim", "Fake yaw limit"]);
            }
            ...
            if (!leftWasPressed && !rightWasPressed) {
                ...
                if (UI.GetValue(["Rage", "Anti Aim", "General", "Key assignment", "AA Direction inverter"])) {
					if (UI.GetValue(["Config", "Scripts", "Keys", "JS Keybinds", "low delta"]))
						UI.SetValue(["Rage", "Anti aim", "Anti aim", "Yaw"], -45);
					else
						UI.SetValue(["Rage", "Anti aim", "Anti aim", "Yaw"], 30);
                }
                //right
                else {
                    UI.SetValue(["Rage", "Anti aim", "Anti aim", "Yaw"], -45);
            ...
            UI.SetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter"], 0);
            UI.SetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter value"], 0);
            if (!leftWasPressed && !rightWasPressed)
                UI.SetValue(["Rage", "Anti aim", "Anti aim", "Lower body yaw target"], 2);
            else
                UI.SetValue(["Rage", "Anti aim", "Anti aim", "Lower body yaw target"], cachelby);
            UI.SetValue(["Rage", "Anti aim", "Anti aim", "Fake yaw limit"], 60);
        }
        else if (!UI.GetValue(["Rage", "Exploits", "Keys", "Key assignment", "Double tap"]) && dtactive) {
            dtactive = false;
            UI.SetValue(["Rage", "Anti aim", "Anti aim", "Yaw"], 0);
            UI.SetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter"], cacheyj);
            UI.SetValue(["Rage", "Anti aim", "Anti aim", "Yaw jitter value"], cacheyjv);
            UI.SetValue(["Rage", "Anti aim", "Anti aim", "Lower body yaw target"], cachelby);
            UI.SetValue(["Rage", "Anti aim", "Anti aim", "Fake yaw limit"], cachefyl);
        }
        ...
        if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Anti-aim display mode"]) == 0)
        ...
        Render.String(sx+50, sy - 13, 1,  ">", (!flip ? [148, 174, 255, Math.max(Math.min(fontalpha, 1), 0) * 255] : [180, 180, 180, Math.max(Math.min(fontalpha, 1), 0) * 120]), fontbig );
        if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Indicate cheat state"])) {
        Render.String(sx + 1, sy + 50 + 1 + offset, 1, UI.GetValue(["Config", "Scripts", "Keys", "JS Keybinds", "low delta"]) ? "LOW DELTA" : (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Enable jitter when running"]) && getVelocity() > 110) ? "JITTER MOVEMENT" : "PERFECT YAW", [0, 0, 0, 255], font);
        Render.String(sx, sy + 50 + offset, 1, UI.GetValue(["Config", "Scripts", "Keys", "JS Keybinds", "low delta"]) ? "LOW DELTA" : (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Enable jitter when running"]) && getVelocity() > 110) ? "JITTER MOVEMENT" : "PERFECT YAW", [145, 164, 235, 255], font);
        offset += 10;
        if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Freestand body yaw"]) == 0) {
            if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Anti-aim display mode"]) == 0) {
            ...
            else if (UI.GetValue(["Rage", "Anti aim", "Anti aim", "Anti-aim display mode"]) == 1 && (leftWasPressed || rightWasPressed)) {
                ...
                Render.String(sx + 1, sy + 50 + 1 + offset, 1, "EXTRA YAW", [0, 0, 0, 255], font);
                Render.String(sx, sy + 50 + offset, 1, "EXTRA YAW", [163, 186, 255, 255], font);
                offset += 10;
            }
        }
        else {
...
function getRandomArbitrary(min, max) {
  ...
  const pitchZeroOnLand = function () {
      if (!UI.GetValue(["Misc.", "View", "General", "Pitch 0 on land"])) return;
      ...
      if (localPlayerFlags == 257 || localPlayerFlags == 261 || localPlayerFlags == 263) {
          groundCounter = groundCounter + 10;
      }
